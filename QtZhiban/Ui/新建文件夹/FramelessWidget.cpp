...
void FramelessWidget::InitWidget() {
    setWindowFlags(Qt::FramelessWindowHint);
    setMouseTracking(true);
}

void FramelessWidget::ShowDialogMaxisize() {
    m_bShowNormal = false;
    showFullScreen();
}

void FramelessWidget::ShowDialogMinisize() {
    showMinimized();
}

void FramelessWidget::ShowDialogNormal() {
    m_bShowNormal = true;
    showNormal();
}

void FramelessWidget::CheckEdge() {
    //计算鼠标距离窗口上下左右有多少距离
...
void FramelessWidget::SlotCloseWidget() {
    close();
}

void FramelessWidget::SlotMinisizeWidget() {
    showMinimized();
}

void FramelessWidget::SlotMaxisizeWidget() {
    if (m_bShowNormal) {
        ShowDialogMaxisize();
    } else {
        ShowDialogNormal();
...
void FramelessWidget::SetMovableArea(QRect rectArea) {
    m_rectMovableArea = rectArea;
}

void FramelessWidget::SetDoubleClickedArea(QRect rectArea) {
    m_rectDoubleClickedArea = rectArea;
}

void FramelessWidget::SetResizeWidget(bool bState) {
    m_bResizeWidget = bState;
}

bool FramelessWidget::PointInArea(const QRect& rectArea, const QPoint& point) {
