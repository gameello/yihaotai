...
CLog::CLOG_LEVEL CLog::logLevel = CLog::INFO;           //Ä¬ÈÏÊÇinfo¼¶
...
void CLog::log(CLOG_LEVEL nLevel, QString fileDesc, QString functionDesc, int lineNum, QString data, ...) {
...
#if defined(Q_OS_WIN)
        int recordLen = _vscprintf(data.toStdString().c_str(), vlist);
        byteArray.resize(recordLen);
#else
        byteArray.resize(1024);
#endif
        vsprintf(byteArray.data() , data.toStdString().c_str(), vlist);
